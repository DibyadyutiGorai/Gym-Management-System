        ...
        private void button_insertp_Click(object sender, EventArgs e)
        {
            ...
            if (isEntryValid())
            {
                try
                {
                    ...
                    cmd.Connection = scon;
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "select * from stocks where Id='" + id.Text + "'";
                    cmd.ExecuteNonQuery();
                    da = new SqlDataAdapter(cmd);
                    dt = new DataTable();
                    da.Fill(dt);
                    ...
                    if (count == 0)
                    {
                        cmd.CommandText = "insert into stocks values('" + id.Text + "','" + productname.Text + "','" + comboBox_category.Text + "','" + dateTimePicker.Text + "','" + comboBox_placedin.Text + "','" + makers.Text + "')";
                        cmd.ExecuteNonQuery();
                        MessageBox.Show("New Item Inserted Sucessfully", "Informaation",
                        MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                    }
                    else
                    {
                        MessageBox.Show("Product id is alreadyRegistered", "Information",
                        MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
                    }
                    cmd.Clone();
                    scon.Close();
                    displayData(query, insert);
                }
                catch (Exception ex)
